...
try:
    # Stack the processed tracks
    tracks_tensor = torch.stack(tracks_to_process, dim=0) # Shape: [num_processed_tracks, 1, samples]

    # Permute/Reshape based on what the model expects
    # Example: If model needs [batch, channels, num_tracks, samples] -> [1, 1, num_processed_tracks, samples]
    # tracks_tensor = tracks_tensor.permute(1, 0, 2).unsqueeze(0)
    # Example: If model needs [batch, num_tracks, channels, samples] -> [1, num_processed_tracks, 1, samples]
...
try:
    mixer = system.model.mixer # Get mixer details
    num_expected_params = mixer.num_params # Get expected number of params PER track

    with torch.no_grad():
        _, params_tensor = system(tracks_tensor) # Get parameters for the processed tracks
